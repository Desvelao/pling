--- Discord channel notifier.
-- Composes a WebhookTransport (does not subclass it) to post Discord
-- webhook messages. The notification is an opaque, caller-defined table:
-- `target_cfg.template(notification)` builds the payload when given;
-- otherwise the whole notification is JSON-encoded into an embed
-- description.
-- @module pling.notifiers.discord
...
-- @treturn table Discord notifier instance
...
function M:send(target_cfg, notification)
  ...
  if type(target_cfg.template) == "function" then
    ...
    payload = {
      embeds = { { description = cjson_safe.encode(notification) } },
    }
  end
